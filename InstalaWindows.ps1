...
# Checagem final: confirma o executavel E o servico
Write-Host "--- Checagem final ---" -ForegroundColor Cyan
$instalouExe = Test-Path -LiteralPath $exeVnc
...
if ($instalouExe -and $servicoVnc) {
    Write-Host "UltraVNC instalado com sucesso. Servico: $($servicoVnc.Name) - Status: $($servicoVnc.Status)" -ForegroundColor Green
}
elseif ($instalouExe -and -not $servicoVnc) {
    Write-Host "Aviso: winvnc.exe existe, mas nenhum servico de VNC foi encontrado." -ForegroundColor Red
}
else {
    Write-Host "Aviso: Instalacao do UltraVNC nao foi confirmada." -ForegroundColor Red
}
Write-Host "--------------------------------------------------------"
#FimInstalaUltraVNC
...
#ConfiguraSIPGonnect
# 1. PERGUNTA INICIAL (repete até vir exatamente "Sim" ou "Nao")
do {
    $resposta = Read-Host "Deseja instalar o SIP (Ramal)? [Sim/Nao]"
    $respostaNormalizada = $resposta.Trim().ToLower() -replace "ã", "a"
} while ($respostaNormalizada -ne "sim" -and $respostaNormalizada -ne "nao")

if ($respostaNormalizada -eq "sim") {
...
function Pergunta-SimNao {
    param([string]$Texto)
    do {
        $resp = Read-Host "$Texto [Sim/Nao]"
        $respNormalizada = $resp.Trim().ToLower() -replace "ã", "a"
    } while ($respNormalizada -ne "sim" -and $respNormalizada -ne "nao")
    return ($respNormalizada -eq "sim")
}

if (Pergunta-SimNao "Deseja instalar as impressoras?") {
    # ====================================================================
    ...
    try {
        ...
        Write-Host "ERRO na preparacao dos drivers: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "Nao sera possivel adicionar impressoras nesta sessao." -ForegroundColor Red
        $InfFiles = $null
    }

    # ====================================================================
    # LAÇO DE REPETIÇÃO (Para adicionar múltiplas impressoras)
    # Deixe o IP vazio e aperte Enter para finalizar - sem pergunta extra.
    # ====================================================================
    if ($InfFiles) {
        while ($true) {
            Clear-Host
            ...
            $IP = Read-Host "Digite o endereco IP da impressora (ou deixe vazio para finalizar)"
            if (-not $IP) { break }

            try {
                $NomeImpressora = Read-Host "Digite o nome de exibição para a impressora (ex: Kyocera Faturamento)"
                if (-not $NomeImpressora) {
                    Write-Host "O nome da impressora nao pode ser vazio. Tentando o proximo IP..." -ForegroundColor Red
                    continue
                }
                ...
                if (-not $ModeloCru) {
                    Write-Host "[$IP] Nao foi possivel obter o modelo via SNMP. Verifique a rede da impressora." -ForegroundColor Red
                    continue
                }
                ...
                if (-not $InfPath -or -not $DriverName) {
                    Write-Host "[$IP] Driver para o modelo '$CoreModel' nao localizado no arquivo INF." -ForegroundColor Red
                    continue
                }
                ...
                if ($Process.ExitCode -ne 0) {
                    Write-Host "[$IP] Falha ao registrar o driver via PrintUI." -ForegroundColor Red
                    continue
                }

                Write-Host "[$IP] Criando a impressora '$NomeImpressora' no Windows..." -ForegroundColor Cyan
                Add-Printer -Name "$NomeImpressora" -DriverName $DriverName -PortName $PortName -ErrorAction Stop
                ...
                Set-PrintConfiguration -PrinterName "$NomeImpressora" -PrintTicketXML $Ticket.OuterXml -ErrorAction Stop
                ...
                Write-Host "[$IP] ERRO inesperado: $($_.Exception.Message)" -ForegroundColor Red
                Write-Host "Seguindo para o proximo IP..." -ForegroundColor Yellow
                continue
            }
        }
    }
